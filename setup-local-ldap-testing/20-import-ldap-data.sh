...
kubectl cp ldap-structure-combined.ldif ldap-testing/$LDAP_POD:$TMP_DIR/

# Verify file was copied
echo -e "${BLUE}🔍 Verifying file contents...${NC}"
kubectl exec -n ldap-testing $LDAP_POD -- wc -l $TMP_DIR/ldap-structure-combined.ldif
RBAC_GROUPS=$(kubectl exec -n ldap-testing $LDAP_POD -- grep -c "app-ocp-rbac-" $TMP_DIR/ldap-structure-combined.ldif)
...
if kubectl exec -n ldap-testing $LDAP_POD -- ldapadd -x -H ldap://localhost:389 \
    ...
    LDAPADD_EXIT=$?
    if [ $LDAPADD_EXIT -eq 0 ]; then
        echo -e "${GREEN}✅ LDAP structure imported successfully!${NC}"
    else
        echo -e "${YELLOW}⚠️  LDAP import completed with exit code $LDAPADD_EXIT${NC}"
        echo "This may indicate some entries already exist (which is okay) or there were warnings"
...
if [[ ! -f configure-acls.ldif ]]; then
    echo -e "${RED}❌ Error: configure-acls.ldif not found in current directory${NC}"
    exit 1
fi

# Copy ACL configuration to pod
kubectl cp configure-acls.ldif ldap-testing/$LDAP_POD:$TMP_DIR/
...
if kubectl exec -n ldap-testing $LDAP_POD -- ldapmodify -x -H ldap://localhost:389 \
    ...
    LDAPMODIFY_EXIT=$?
    if [ $LDAPMODIFY_EXIT -eq 0 ]; then
        echo -e "${GREEN}✅ ACLs configured successfully!${NC}"
    else
        echo -e "${YELLOW}⚠️  ACL configuration completed with exit code $LDAPMODIFY_EXIT${NC}"
        echo "This may indicate ACLs were already configured (which is okay)"
    fi
else
    LDAPMODIFY_EXIT=$?
...
RBAC_COUNT=$(kubectl exec -n ldap-testing $LDAP_POD -- ldapsearch -x -H ldap://localhost:389 \
    ...
    -w "bindpassword123" \
    -b "ou=Groups,dc=ephico2real,dc=com" \
    "(&(objectClass=groupOfNames)(cn=app-ocp-rbac-*))" cn 2>/dev/null | grep -c "^cn:" || echo "0")

echo -e "${GREEN}✅ Found $RBAC_COUNT RBAC groups (app-ocp-rbac-*)${NC}"
...
USER_COUNT=$(kubectl exec -n ldap-testing $LDAP_POD -- ldapsearch -x -H ldap://localhost:389 \
    ...
    "(objectClass=inetOrgPerson)" cn 2>/dev/null | grep -c "^cn:" || echo "0")

echo -e "${GREEN}✅ Found $USER_COUNT users in People OU${NC}"
...
echo "  ✓ Created $RBAC_COUNT RBAC groups (app-ocp-rbac-*)"  
