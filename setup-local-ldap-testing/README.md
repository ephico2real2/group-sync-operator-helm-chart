...
| `ldap-rbac-groups-ocp-admins.ldif` | `app-ocp-rbac-ocp-cluster-admin` (ClusterRole `admin`, via GroupConfig) and `app-ocp-rbac-ocp-keycloak-admin` — Keycloak admins with **no** OpenShift permission; `sarah.jones` is the test case |
